...
方案  				  			优点 			缺点						加密等级
json数据加密 对称加密   			简单			前端暴露密码				*
json数据加密 非对称加密   			快速			后端每次要返回对接公钥		**
传输协议加密二进制（protobuf.js）	数据接口爬取成本高		扩展性差，性能	***
随机加密算法（后端根据随机规则生成解密js）反爬程度高				成本高					*****
...
| json数据加密 对称加密 | 简单     | 前端暴露密码  |* |
| json数据加密 非对称加密 | 快速    | 后端每次要返回对接公钥  | \*\*|
| 传输协议加密二进制（protobuf.js）    | 数据接口爬取成本高    | 扩展性差，损坏一定后端性能  |\*\*\*|
| 随机加密算法（后端根据随机规则生成解密js）   |反爬成本效果最好 | 成本高    | \*\*\*\*\*|
