/* esm.sh - esbuild bundle(vue@2.7.14) es2022 development */
import __Process$ from '/cdn/v99/node_process.js';
var __global$ = globalThis || (typeof window !== 'undefined' ? window : self);
var __setImmediate$ = (cb, ...args) => setTimeout(cb, 0, ...args);
// esm-build-a1ae51f04bb02976ddc491bfd3fc1a964234ba53-3556b8d4/node_modules/vue/dist/vue.runtime.esm.js
...
var isServerRendering = function() {
  if (_isServer === void 0) {
    if (!inBrowser && typeof __global$ !== 'undefined') {
      _isServer = __Process$ && __Process$.env.VUE_ENV === 'server';
    } else {
      _isServer = false;
    }
  }
...
function doWatch(source, cb, _a) {
  ...
  if (isServerRendering()) {
    ...
    if (!cb) {
      ...
      call(cb, WATCHER_CB, [getter(), isMultiSource ? [] : void 0, onCleanup]);
    }
    return noop;
  }
  ...
  watcher.run = function() {
    ...
    if (cb) {
      ...
      if (
        ...
        call(cb, WATCHER_CB, [newValue, oldValue === INITIAL_WATCHER_VALUE ? void 0 : oldValue, onCleanup]);
        oldValue = newValue;
      }
    } else {
      watcher.get();
...
if (typeof Promise !== 'undefined' && isNative(Promise)) {
  ...
  isUsingMicroTask = true;
} else if (typeof __setImmediate$ !== 'undefined' && isNative(__setImmediate$)) {
  timerFunc = function() {
    __setImmediate$(flushCallbacks);
  };
} else {
  timerFunc = function() {
        ...
        tree
          .map(function(vm2, i) {
            return ''
              ...
              .concat(
                isArray(vm2)
                  ? ''.concat(formatComponentName(vm2[0]), '... (').concat(vm2[1], ' recursive calls)')
                  : formatComponentName(vm2)
              );
          })
          .join('\n')
      );
...
function normalizeProps(options, vm) {
  ...
  if (isArray(props2)) {
    ...
    warn(
      'Invalid value for option "props": expected an Array or an Object, ' + 'but got '.concat(toRawType(props2), '.'),
      vm
    );
  }
  options.props = res;
  ...
  if (isArray(inject2)) {
    ...
    warn(
      'Invalid value for option "inject": expected an Array or an Object, ' +
        'but got '.concat(toRawType(inject2), '.'),
      vm
    );
  }
}
...
if (true) {
  ...
  hasHandler_1 = {
    has: function(target2, key) {
      ...
      var isAllowed =
        allowedGlobals_1(key) || (typeof key === 'string' && key.charAt(0) === '_' && !(key in target2.$data));
      if (!has2 && !isAllowed) {
        if (key in target2.$data) warnReservedPrefix_1(target2, key);
        else warnNonPresent_1(target2, key);
      }
        ...
        if (!isRoot && !isUpdatingChildComponent) {
          warn(
            "Avoid mutating a prop directly since the value will be overwritten whenever the parent component re-renders. Instead, use a data or computed property based on the prop's " +
              'value. Prop being mutated: "'.concat(key2, '"'),
            vm
          );
        }
      });
  ...
  function assertNodeMatch(node, vnode, inVPre) {
    if (isDef(vnode.tag)) {
      return (
        ...
        (!isUnknownElement2(vnode, inVPre) && vnode.tag.toLowerCase() === (node.tagName && node.tagName.toLowerCase()))
      );
    } else {
